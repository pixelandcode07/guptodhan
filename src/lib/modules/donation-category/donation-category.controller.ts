...
} from "./donation-category.validation";
import {
  DonationCategoryServices,
...
const createCategory = async (req: NextRequest) => {
  ...
  const result = await DonationCategoryServices.createCategoryInDB(
    validatedData
  );
  return sendResponse({
    success: true,
...
const updateCategory = async (
  ...
  const result = await DonationCategoryServices.updateCategoryInDB(
    id,
    validatedData
  );
...
const getCategoryByIdForAdmin = async (
  _req: NextRequest,
  { params }: { params: Promise<{ id: string }> }
) => {
  await dbConnect();
  const { id } = await params;
  const result = await DonationCategoryServices.getCategoryByIdForAdminFromDB(
    id
  );
  return sendResponse({
    success: true,
...
// Reorder donation category (drag-and-drop)
const reorderDonationCategory = async (req: NextRequest) => {
  await dbConnect();
  ...
  const result = await reorderDonationCategoryService(orderedIds);

  return sendResponse({
    success: true,
    statusCode: StatusCodes.OK,
    message: result.message || "Donation Category reordered successfully!",
    data: null,
  });
};
