...
import { createSocialLinkSchema, updateSocialLinkSchema } from './socialLink.validation';
...
const updateSocialLink = async (req: NextRequest, { params }: { params: { id: string } }) => {
    await dbConnect();
    const { id } = params;
    const body = await req.json();
    const validatedData = updateSocialLinkSchema.parse(body);
    const result = await SocialLinkServices.updateSocialLinkInDB(id, validatedData);
...
const deleteSocialLink = async (_req: NextRequest, { params }: { params: { id: string } }) => {
    await dbConnect();
    const { id } = params;
