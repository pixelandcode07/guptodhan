...
const loginUser = async (payload: TLoginUser) => {
  ...
  // ✅ MAGIC FIX: Added hasPassword to JWT payload so NextAuth session can read it
  const jwtPayload = {
...
const serviceProviderSendRegistrationOtp = async (email: string) => {
  ...
  const existingUser = await User.findOne({ email });
  if (existingUser) throw new Error('এই ইমেইলটি ইতিমধ্যে ব্যবহার করা হয়েছে।');
...
const serviceProviderLogin = async (payload: TLoginUser) => {
  ...
  return {
    ...
    user: {
      ...
      hasPassword: !!password, // ✅ Returned for consistency
    },
  };
};
...
const serviceProviderSendForgotPasswordOtp = async (email: string) => {
  ...
  if (user.role !== 'service-provider') {
    throw new Error('এই ইমেইলটি সার্ভিস প্রোভাইডার অ্যাকাউন্টের সাথে যুক্ত নয়।');
  }
