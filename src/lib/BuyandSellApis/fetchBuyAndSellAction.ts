...
export async function markAdAsPending(adId: string) {
  const token = await ensureAdmin();
  ...
  try {
    ...
    revalidatePath(REVALIDATE_PATH);
    return { success: true, message: 'Ad moved back to pending!' };
  } catch (error: any) {
    return { success: false, message: error.response?.data?.message || 'Failed to mark as pending' };
  }
}

export async function markAdAsActive(adId: string) {
  const token = await ensureAdmin();
  ...
  try {
    ...
    revalidatePath(REVALIDATE_PATH);
    return { success: true, message: 'Ad activated successfully!' };
  } catch (error: any) {
    return { success: false, message: error.response?.data?.message || 'Failed to activate ad' };
  }
}

export async function markAdAsInactive(adId: string) {
  const token = await ensureAdmin();
  ...
  try {
    ...
    revalidatePath(REVALIDATE_PATH);
    return { success: true, message: 'Ad marked as inactive!' };
  } catch (error: any) {
    return { success: false, message: error.response?.data?.message || 'Failed to mark as inactive' };
  }
}