...
// import { Trash, CheckCircle, XCircle, Mail, Phone, ShieldCheck, UserCheck } from "lucide-react";
...
//                 // Background update API endpoint (Assuming endpoint structure)
//                 const apiUrl = `/api/v1/auth/service-providers/${userId}/toggle-status`;
...
//                             success: `Provider ${user.isActive ? 'deactivated' : 'activated'}!`,
//                             error: "Failed to update status",
//                             style: { background: "#fff", color: "#000" } // White toast update
//                         }
//                     );
...
//                             prev.map(item =>
//                                 item._id === userId ? { ...item, isActive: !user.isActive } : item
//                             )
//                         );
//                     }
...
//                 const confirmed = await confirmDelete(`Delete provider "${user.name}"?`);
//                 if (!confirmed) return;

//                 try {
//                     await toast.promise(
//                         axios.delete(`/api/v1/auth/service-providers/${userId}`),
//                         {
//                             loading: "Deleting...",
...
//                         }
//                     );
//                     if (setData) setData((prev: IProvider[]) => prev.filter(item => item._id !== userId));
//                 } catch (error) { console.error(error); }
//             };
  ...
  },
  {
    ...
    cell: ({ row, table }) => {
      ...
      const handleStatusUpdate = async (action: "approve" | "reject") => {
        const apiUrl = `/api/v1/service-section/service-provider/${action}/${userId}`;
        ...
        try {
          ...
          if (setData) {
            setData((prev: IProvider[]) =>
              prev.map((item) =>
                item._id === userId ? { ...item, isActive: newActiveStatus } : item
              )
            );
          }
        } catch (error) {
