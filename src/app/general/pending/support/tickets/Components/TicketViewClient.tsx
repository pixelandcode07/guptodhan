...
export default function TicketViewClient({ initialTicket }: TicketViewClientProps) {
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      ...
      setTicket(res.data.data); // সম্পূর্ণ টিকেট অবজেক্ট দিয়ে স্টেট আপডেট করুন
  ...
  const handleSendReply = async () => {
    ...
    try {
      ...
      formData.append('sender', 'admin'); // যেহেতু এটি অ্যাডমিন প্যানেল
      ...
      setTicket(res.data.data); // নতুন কথোপকথন সহ টিকেট অবজেক্ট দিয়ে স্টেট আপডেট করুন
      setReplyMessage('');
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      {/* --- বাম কলাম: টিকেট তথ্য এবং স্ট্যাটাস --- */}
      <div className="lg:col-span-1 space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            {/* এই অংশটি তৈরি করতে হলে আপনাকে স্ট্যাটাস পরিবর্তনের লগ সেভ করতে হবে */}
            <p className="text-sm text-gray-500">No status changes yet.</p>
          </CardContent>
        </Card>
      </div>

      {/* --- ডান কলাম: কথোপকথন (Conversation) --- */}
            ...
            {ticket.conversation.map((msg, index) => (
              <div key={index} className={`flex ${msg.sender === 'admin' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-lg p-4 rounded-lg ${
                  msg.sender === 'admin' ? 'bg-blue-600 text-white' : 'bg-gray-100'
                }`}>
                  ...
                  {msg.attachment && (
                    <a 
                      ...
                      className="mt-2 inline-flex items-center gap-2 text-sm font-medium text-blue-200 hover:text-white"
                    >
