...
export default function RearrangeStoragesClient() {
  return (
    <ReorderList
      title="Rearrange Storage Types"
      description="Drag and drop to change the order. Click 'Save Order' to persist changes."
      fetchUrl="/api/v1/product-config/storageType"
      patchUrl="/api/v1/product-config/storageType"
      mapItem={(raw: unknown) => {
        const s = raw as { _id?: string; ram?: string; rom?: string; status?: string };
        if (!s?._id) return null;
        return {
          id: String(s._id),
          label: `${s.ram || 'N/A'} / ${s.rom || 'N/A'}`,
          extra: s?.status ? (
