...
export default function RearrangeSizesClient() {
  return (
    <ReorderList
      title="Rearrange Sizes"
      description="Drag and drop to change the order. Click 'Save Order' to persist changes."
      fetchUrl="/api/v1/product-config/productSize"
      patchUrl="/api/v1/product-config/productSize"
      mapItem={(raw: unknown) => {
        const s = raw as { _id?: string; name?: string; status?: string };
        if (!s?._id) return null;
        return {
          id: String(s._id),
          label: String(s.name || 'Unnamed Size'),
          extra: s?.status ? (
