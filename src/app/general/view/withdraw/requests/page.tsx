...
import { Withdraw_req, withdraw_req_columns } from '@/components/TableHelper/withdraw_req_columns'
import { Input } from '@/components/ui/input'



const getWithdrawalRequest = async (): Promise<Withdraw_req[]> => {
    return [
        {
            serial: '1',
...
export default async function WithdrawRequests() {
    const withdrawalReq = await getWithdrawalRequest()
    return (
        <>
            <div >
            ...
            <div>
                <DataTable columns={withdraw_req_columns} data={withdrawalReq} />
            </div>
        </>
    )
}
