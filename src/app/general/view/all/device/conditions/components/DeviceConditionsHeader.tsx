...
import { Plus, Smartphone, Move } from "lucide-react";
import Link from "next/link";

interface DeviceConditionsHeaderProps {
  onAddClick: () => void;
}

export default function DeviceConditionsHeader({ onAddClick }: DeviceConditionsHeaderProps) {
  return (
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-gray-600 mt-1">
              Manage and organize your device conditions
            </p>
          </div>
        </div>
        ...
        <div className="flex-shrink-0 flex flex-col sm:flex-row gap-2 sm:gap-3">
          <Button variant="outline" asChild className="h-10 sm:h-auto text-xs sm:text-sm">
            <Link href="/general/view/all/device/conditions/rearrange">
              <Move className="w-4 h-4 mr-2" />
              <span className="hidden sm:inline">Rearrange Device</span>
          ...
          >
            ...
            <span className="hidden sm:inline">Add New Device Condition</span>
            <span className="sm:hidden">Add Condition</span>
          </Button>
        </div>
      </div>
    </div>
