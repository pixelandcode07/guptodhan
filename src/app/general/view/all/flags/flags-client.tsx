...
export default function FlagsClient() {
  ...
  const onDelete = useCallback(async (flag: Flag) => {
    if (window.confirm("Are you sure you want to delete this flag?")) {
      try {
        await axios.delete(`/api/v1/product-config/productFlag/${flag._id}`, {
          headers: {
            ...(token ? { Authorization: `Bearer ${token}` } : {}),
            ...(userRole ? { "x-user-role": userRole } : {}),
          },
  ...
  return (
    <div className="space-y-4">
      ...
      <DataTable columns={columns} data={filteredFlags} loading={loading} />
