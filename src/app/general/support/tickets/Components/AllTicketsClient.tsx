...
import { Check, Eye, Loader2, Plus, Trash2, XIcon, Search as SearchIcon } from 'lucide-react';
...
export default function AllTicketsClient({ initialTickets, initialStats }: AllTicketsClientProps) {
  ...
  const refreshData = async () => {
    try {
        const [ticketsRes, statsRes] = await Promise.all([
            axios.get('/api/v1/crm-modules/support-ticket', { headers: { Authorization: `Bearer ${token}` } }), // সব টিকেট আনা হচ্ছে
  ...
  const handleDelete = (id: string) => {
    ...
    toast("Are you sure you want to delete this ticket?", {
        ...
        cancel: { label: "Cancel", onClick: () => {} } // ✅ sonner-এর জন্য onClick যোগ করা হয়েছে
    });
  };
  ...
  const statCards = [
    { title: "All Tickets", count: stats.all, color: "blue", href: "/general/support/tickets" },
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {statCards.map(card => (
          <Link href={card.href} key={card.title}>
            <Card className={`hover:shadow-lg transition-shadow ${card.title === 'All Tickets' ? 'ring-2 ring-blue-600' : 'border-gray-200'}`}>
