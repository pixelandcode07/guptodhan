...
interface Props {
  ...
  category: { name: string; slug: string; adCount: number }; // ✅ Slug added
  categoryId: string;
}
...
export default function CategoryItemsClient({
  ...
  // ✅ FIX: Initialize Price State Safely
  const [price, setPrice] = useState([priceRange.min || 0, priceRange.max || 100000]); 
  const [isFilterOpen, setIsFilterOpen] = useState(false);

  // ✅ FIX: Sync price when props change (on category switch)
  useEffect(() => {
    if (priceRange.max > 0) {
  ...
  const applyFilters = useCallback(async () => {
    ...
    try {
      ...
      // 🔥 FIX: Use the Slug API Endpoint for filtering
      // This ensures we keep using the optimized aggregated API logic
      const res = await axios.get(`/api/v1/public/classifieds/category-page/${category.slug}?${params.toString()}`);
      
      // The API returns { data: { ads: [...] } } structure
      if (res.data.success) {
         setAds(res.data.data.ads || []);
      }
    } catch (error) {
      console.error('Filter error:', error);
      // Fallback: don't clear ads on error to keep UX smooth
    } finally {
      setLoading(false);
    }
  }, [searchTerm, category.slug, selectedSubCats, selectedLocations, selectedBrands, price]);

  // Debounce Effect for API Calls
  useEffect(() => {
    // Avoid initial double fetch, rely on initialAds
    const timer = setTimeout(() => {
        // Only fetch if filters are actually applied or changed from default
        // Logic can be refined, but simple debounce works well here
        applyFilters(); 
    }, 600);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 py-4 sm:py-6">
        ...
        <div className="flex gap-4 lg:gap-6">
          {/* Desktop Sidebar */}
          <aside className="hidden lg:block w-80 flex-shrink-0 sticky top-24 h-[calc(100vh-100px)] overflow-y-auto scrollbar-hide">
            <FilterSidebar />
          </aside>
          ...
          <main className="flex-1">
            ...
            <div className="hidden lg:flex justify-between items-center mb-4 bg-white p-3 rounded-lg shadow-sm">
              ...
              <Button variant="outline" className="border-blue-600 text-blue-600 hover:bg-blue-50">
                <Link href="/buy-sell" className="flex items-center">
            ...
            <div className="flex justify-between items-center mb-4">
              ...
              <Badge variant="secondary" className="text-sm px-3 py-1">
                {loading ? 'Updating...' : `${ads.length} Ads`}
              </Badge>
            </div>

            {/* Product Grid */}
            {loading && ads.length === 0 ? (
               <div className="text-center py-20">Loading ads...</div>
            ) : ads.length === 0 ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4">
                {ads.map(ad => (
                  <Link
                    key={ad._id}
                    href={`/buy-sell/ad-details/${ad._id}`}
                    className="bg-white rounded-xl shadow-sm border border-gray-100 hover:shadow-lg hover:border-blue-200 transition-all duration-300 overflow-hidden group flex flex-col h-full"
                  >
                    {/* Image */}
                    <div className="relative aspect-square bg-gray-50 overflow-hidden">
                      ...
                      />
                      <div className="absolute top-2 right-2 bg-white/90 backdrop-blur-sm px-2 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wide text-gray-700 shadow-sm">
                        {ad.condition}
                      </div>
                    </div>
                    ...
                    <div className="p-3 flex flex-col flex-grow">
                      <h3 className="font-semibold text-sm line-clamp-2 mb-1 text-gray-800 group-hover:text-blue-600 transition-colors">
                        {ad.title}
                      </h3>
                      ...
                      <div className="mt-auto space-y-2">
                        <div className="flex items-center justify-between">
                           {ad.brand && (
                            <span className="text-[10px] px-1.5 py-0.5 bg-gray-100 rounded text-gray-600 truncate max-w-[80px]">
                              {ad.brand}
                            </span>
                          )}
                          <div className="flex items-center text-[11px] text-gray-500">
                            <MapPin className="w-3 h-3 mr-0.5" />
                            <span className="truncate max-w-[80px]">{ad.district}</span>
                          </div>
                        </div>
