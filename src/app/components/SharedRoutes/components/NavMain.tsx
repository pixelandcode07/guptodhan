...
export default function NavMain() {
  ...
  useEffect(() => {
    const fetchLatestProfile = async () => {
      try {
        ...
        if (response.data?.success && response.data?.data) {
          setProfileImage(response.data.data.profilePicture ?? null);
        }
      } catch (error) {
    ...
    // Fetch latest profile when NavMain loads
    if (session?.user) {
      fetchLatestProfile();
    }
  ...
  return (
    <div>
      ...
      onOpenChange={handleOpenChange}
          >
            <ul className="flex flex-wrap lg:flex-nowrap gap-2 lg:gap-4 items-center justify-end">
              ...
              {/* Cart & Wishlist */}
              <li className="scale-90 lg:scale-100">
                <CartIcon />
              </li>
              <li className="scale-90 lg:scale-100">
                <WishlistIcon />
                ...
                <li className="flex flex-col justify-center items-center text-[#00005E] font-medium">
                  <Link href="/home/UserProfile">
                    <button className="flex flex-col items-center gap-0.5 outline-none group cursor-pointer">
                      ...
                      <span className="text-[#00005E] text-[10px] lg:text-[11px] font-medium">
                        Account
                      </span>
                    </button>
                  </Link>
              ...
              <li className="hidden xl:flex items-center">
                <DropdownMenu>
                  ...
                  <DropdownMenuContent
                    align="end"
                    className="w-64 p-2 shadow-2xl border-gray-100"
                  >
                    ...
                    <DropdownMenuItem asChild>
                      <Link
                        href="tel:+8801816500600"
                        className="cursor-pointer flex items-center gap-3 py-2 text-blue-600"
                      >
                        <PhoneCall size={18} />
      ...
      {/* ═══════════════════════════════════════════════════════════════
          ...
          — Only ONE <SearchBar /> instance here (previously there were
            two mounted at the same time, which caused the duplicate
            fetch / race-condition bug where the dropdown showed
            "No products found" even though products existed).
          — onSearch={() => setMobileSearchOpen(false)} closes the
            overlay automatically after any navigation from SearchBar.
      ═══════════════════════════════════════════════════════════════ */}
      <AnimatePresence>
        {mobileSearchOpen && (
          <motion.div
